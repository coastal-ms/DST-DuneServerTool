...
const ACTIONS: ActionDef[] = [
  ...
  { id: 'reset-journey', group: 'Progression', label: 'Reset Journey', icon: 'RefreshCw', offlineOnly: true,
    doubleConfirm: true,
    rowNote: 'Full quest restart: journey nodes, NPE/journey/contract tags, and contract items. Faction state is preserved. Offline.',
    confirm: p => `RESET ${p.name}'s entire journey and restart every quest from the beginning? Journey nodes, NPE/journey/contract tags, and contract items will be removed. Faction state is preserved. This cannot be undone.\n\n` +
...
export function JourneySection({ player, canWrite, demo, refreshKey, flash, onChanged }: SectionProps) {
  ...
  const resetAll = () => {
    if (!window.confirm(
      `RESET ${player.name}'s entire journey and restart every quest from the beginning? Journey nodes, NPE/journey/contract tags, and contract items will be removed. Faction state is preserved. This cannot be undone.\n\n` +
      `This is the FIRST of two confirmations. If you continue, the next step asks you to type an acknowledgement before the journey is reset.`
    )) return
    const typed = window.prompt(
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {canWrite && (
          <button type="button" className="btn-secondary shrink-0 text-xs text-error" disabled={busy || isOnline} onClick={resetAll}
            title={isOnline ? 'Player must be offline' : 'Delete journey nodes, NPE/journey/contract tags, and contract items; preserve faction state'}>
