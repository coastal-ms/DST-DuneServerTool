...
const ACTIONS: ActionDef[] = [
  ...
  { id: 'repair-orphaned-building-pieces', group: 'Items', label: 'Repair Orphaned Building Pieces', icon: 'Wrench',
    rowNote: 'Fixes a 2026-09-22 Funcom patch bug: old Gunner/Sentinel building-set pieces can crash the construction tool. Only affects characters that unlocked that set before the patch. Player must be offline.',
    confirm: p => `Repair orphaned building pieces on ${p.name}?\n\nRemoves the specific dead Gunner/Sentinel building-set piece ids a 2026-09-22 Funcom patch orphaned, which can crash the client's construction tool. Only those exact ids are touched — nothing else. No-op if none are present. Player must be offline.`,
    run: p => repairOrphanedBuildingPieces(p.id) },
