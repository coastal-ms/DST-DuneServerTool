...
$script:DuneGameConfigSchema = @(
    ...
    @{ Section=$script:DuneGcSecCoriolis; Key='m_CycleStartHour'; File='game'; Type='int'; Min=0; Max=23; Default='5'; Label='Coriolis Cycle Start Time'; Help='Whole-hour GMT/UTC start time for the Coriolis cycle. DST shows the matching current browser-local time while storing the GMT hour required by the game. Because the game stores fixed GMT, the local time shifts when daylight saving changes. Use Apply INIs & restart after saving.'; ClientApply=$true; Category='Storm Cycle' }
