...
## Advanced CVar catalog

`advanced-cvars.json` contains the complete recovered console-variable inventory
used by Experimental Lab. Regenerate it after decoding a new server build:

```powershell
pwsh tools/Build-AdvancedCvarCatalog.ps1 -InputPath <cvars-decoded.json>
```

Game Config excludes keys already surfaced by DST. Catalog risk labels are
triage metadata, not proof a control is safe or functional.
