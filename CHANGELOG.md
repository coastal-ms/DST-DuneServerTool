...
- **Skorda contract card now clears from the journey system, not just tags.** Even after tags and the active contract item were fixed, the *Skorda's Last Stand* card could remain in the Arrakeen Contract tab because the card is driven by the **journey system**, not tags. The `...Complete "Track Down Skorda" Contract` journey node was left at `complete_condition_state='true'` (an *active, condition-met* state the game renders as a live card), whereas a naturally-completed character has that node **consumed to `{}`**. The Complete Contract action now also consumes stuck faction-storyline contract-completion nodes to that turned-in state, so the card drops and Thufir Hawat's dialogue ungates. Fully compatible with **Reset Faction**, which blanket-resets ClimbTheRanks nodes and overrides this state on a wipe.
