...
- **Connection check (P34): one-click Fix-it button on the "inter-world gateway is private" verdict.** When `dune.farm_state.igw_addr` is a pod/container-private address, the public IP itself is fine — only the last IGW registration is stale, and the summary already told the user to restart the battlegroup. The new button (`POST /api/public-ip/repair-igw`) recycles the game-server pods (Overmap + Survival + any running Deep Desert) in a single gesture so the operator re-registers `farm_state` with the node's address. Cheaper than the full Public IP apply (no `settings.conf` rewrite, no `change-battlegroup-ip`, no NAT touch), and it disconnects only players currently on a map. Backend adds `Repair-DuneIgwPrivate` in `PublicIp.ps1`, extends `$DuneRestartablePods` in `Maps.ps1` with an `overmap` entry (matches `-sg-overmap-pod-`), and reuses the existing `Restart-DuneMapPods` primitive under an `ondemand-maps` lock.
