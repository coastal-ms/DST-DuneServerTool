...
- **“Player config” button — everything your players need, in one place.** Game Config and the new Experimental page each get a button that shows every setting a player must also set on their own PC, gathered from all sections at once. `Game.ini` and `Engine.ini` are shown side by side with their file locations, and a single copy button takes both. Only settings you have actually changed are listed — defaults and server-only settings are left out, so nobody pastes lines that do nothing.
