...
- **Skorda contract card now clears from the journey system, not just tags.** Even after tags and the active contract item were fixed, the *Skorda's Last Stand* card could remain in the Arrakeen Contract tab because the card is driven by the **journey system**, not tags. The `...Complete "Track Down Skorda" Contract` journey node was held open by its **reveal** condition (`reveal_condition_state='true'`) — the state the game renders as a live card — while a naturally-completed character has that node **consumed to `{}` on both its complete and reveal conditions**. The Complete Contract action now consumes stuck faction-storyline contract-completion nodes on **both** fields to that turned-in state, so the card drops and Thufir Hawat's dialogue ungates. Because `{}` is the game's own natural post-turn-in state (a normal character carries many nodes at `reveal='{}'`), this stays fully compatible with **Reset Faction** and storyline replay after a faction wipe — no stranded state.
